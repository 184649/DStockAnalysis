...
- **速度(リアルタイム性)**:
  - 個別取得は4ソース(株探×2・Yahoo・IRBANK×2)を**並行取得**し、1銘柄あたり約0.7〜1.3秒。
  - 巡回取得は**同時並行**(`Concurrency`、既定4)で全銘柄を高速に取得(自社株買い・連続増配・増配率も反映)。
  - 一覧表は**行の仮想化**(画面に見える分だけ描画)で、3,739行でもスクロール・並べ替えが軽快。
- **自社株買い額**は IR BANK「自己株式の取得」から取得します。
- 画面更新が反映されない時は、`appsettings` 反映のため**サーバ再起動**＋ブラウザ再読み込み(静的ファイルは毎回再検証)。

### 暫定取得 と 実取得(一覧の表示について)

一覧を素早く埋めるため、起動直後に Yahoo 一括取得で **株価・PER・PBR・配当利回り・EPS** だけを先に表示します(=**暫定**、コードに「暫」)。
この段階では利益率・ROA・各CF・自己資本比率・成長率や**バフェットスコアは「-」**です(財務指標が無いため、誤った低評価を出しません)。

バックグラウンド巡回(`Enabled=true`)または銘柄を開く(`OnDemand`)と、株探の会社予想・業績/財務ページから財務指標を取得して **実取得** に昇格し、全指標とスコアが表示されます。`Enabled=true` の間は、全銘柄が順次 暫定→実取得 に更新されます(robots 順守・低頻度のため全件完了には時間がかかります)。

> 取得する指標を増やした場合は内部の取得スキーマ版が上がり、旧データの銘柄は次に開いた時/巡回時に自動で再取得されます。
...
| バフェットスコア | バフェット型の総合採点(100点)。6サブスコア(事業耐久力/収益力/財務安全性/成長安定性/資本配分/割安性)を、**Claude作成の教師データ45件に適合するよう最適化した重み**で合成。欠損は除外して重み再配分、データ信頼度・赤字/財務危険/サンプル等で上限、S〜Eランク。業種プロファイル(通常/金融/総合商社)で評価軸を切替、Sは厳格条件。**高評価要因・減点要因・ランク判定理由**を表示。アプリの主軸=バフェット評価。詳細 → `docs/バフェットスコア算出方法.md` |
...
- 合計 **146 件・0 失敗**。詳細は `docs/単体試験項目書.md` / `docs/結合試験項目書.md`。
...
| 種別 | ファイル |
|---|---|
| バフェットスコア算出方法 | `docs/バフェットスコア算出方法.md` |
