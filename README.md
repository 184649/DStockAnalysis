...
>   (株価/PER/PBR/ROE/配当利回り/時価総額/自己資本比率/EPS 等)を **オンデマンドで取得**して表示します(取得済みはキャッシュで即時)。
>   株価はみんかぶの構造化データ(JSON-LD)、自己資本比率は IR BANK のバー表示から取得し、
>   配当性向・1株配当・MIX係数・BPS は取得した実値から算出します。
