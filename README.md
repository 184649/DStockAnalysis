...
>   取得する指標: 株価(現在値)・PER・PBR・EPS・BPS・配当利回り・配当性向・1株配当・ROE・純利益率・増収率・純利益成長率・
>   営業CF・フリーCF・営業CFマージン・有利子負債比率(以上 **Yahoo! ファイナンスの構造化JSON**)、時価総額(**みんかぶ**)、
>   自己資本比率(**IR BANK**)、MIX係数(PER×PBR)。
>   - 営業利益率は信頼できる無料の出典が無い(Yahoo の値は日本株で実態と乖離)ため**未取得**のままにします(誤った値を出さない方針)。
