...
| バフェットスコア | バフェットの思考に基づく独自採点(100点)。6サブスコアの重み付き和: 事業耐久力25%/収益力20%/財務安全性15%/成長安定性15%/資本配分10%/割安性15%。欠損は除外して重み再配分し、データ信頼度・赤字/財務危険/サンプル等で上限。S〜Eランク・判定コメント付き。金融業は専用基準。一覧/個別/比較で表示。詳細 → `docs/バフェットスコア算出方法.md` |
...
- 合計 **133 件・0 失敗**。詳細は `docs/単体試験項目書.md` / `docs/結合試験項目書.md`。
