...
> **重要(実データ取得の方針)**: これらのサイトは公開 API を提供しておらず、利用規約・アクセス負荷・
> 再配布の観点から、本アプリは **全銘柄ぶんを自動スクレイピングして取り込むことはしません**。
> そこで次の **半自動フロー**で実数値を反映します(詳細は `docs/データ取得手順.md`)。

### 実データ取得フロー(要点)
1. **全銘柄リスト(実データ)は自動**: `tools\update_master.bat` か「全銘柄更新(JPX)」で最新化。
2. **テンプレ出力**: アプリ「テンプレ出力」で全銘柄前埋めの記入用 CSV を作成。
3. **実数値を記入**: 上表のリンク(または `tools\open_sources.bat 7203`)で各銘柄の最新値を確認して記入。
4. **CSV取込**: 記入した **列だけ**が実データで上書き(列単位マージ。未記入列は維持、新規コードは追加)。
5. **保存**: 次回起動時も再現。

> 全銘柄ぶんを一度に埋める必要はありません。注目銘柄から少しずつ実データ化できます。
> 完全自動(実財務まで)を望む場合は、無料登録制の **J-Quants API / EDINET API** が選択肢です
> (`Services` に取得クラスを足すだけで連携できる構造)。
