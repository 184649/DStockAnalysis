...
>   (株価/PER/PBR/ROE/配当利回り/配当性向/EPS/時価総額/自己資本比率)を **オンデマンドで取得**して表示します(取得済みはキャッシュで即時)。
>   主要指標は **Yahoo! ファイナンスの構造化データ(現在値)** を採用。時価総額はみんかぶ(百万円)、自己資本比率は IR BANK、
>   MIX係数・BPS・1株配当は実値から算出します。
