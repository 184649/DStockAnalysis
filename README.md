...
> 代わりに、上記の **銘柄別リンクで最新の実数値へ即アクセス** できるようにし、確認した数値を
> **CSV 取込**でアプリに反映する運用としています。
> 全銘柄の実財務を自動取得したい場合は、公式かつ無料の **J-Quants API**(要・無料登録)が適しています。
> 本アプリは `Services` に取得サービスを追加するだけで連携できる構造です(将来拡張)。
