...
> PER・ROE・配当などの財務指標は含まれません。
> - **WPF デスクトップ版**は全銘柄に **もっともらしいサンプル(擬似)指標値** を自動生成して表示し、スクリーニングや比較をすぐ体験できます(実データは CSV取込で置換)。
> - **Web 版はサンプル値を使いません。** 未取得の銘柄は空欄で表示し、Yahoo! ファイナンス等から **実データを取得**して埋めます(詳細は下の「Web 版の特徴」)。
...
> - **擬似(サンプル)値は一切使いません。** 起動直後は全銘柄が「未取得」(指標は空欄)で、実データを取得した銘柄だけ値を表示します。
> - **指標は実データを取得**します。個別分析で銘柄を開いた瞬間に、その銘柄の実値を **オンデマンドで取得**して表示します(取得済みはキャッシュで即時)。
>   出典は日本の投資情報サイトの表示(**会社予想ベース**)に合わせています:
>   - **株探**: PER・PBR・配当利回り・時価総額(会社予想ベース。日本の投資家が見る値に一致)。
>   - **Yahoo! ファイナンス**(構造化JSON): **株価(現在値)**・ROE・純利益率・営業CF・フリーCF・有利子負債比率。
>     (株価は Yahoo の chart API から取得。株探のテキストは銘柄により株価位置が不安定なため使わない。)
> - **株価は常に最新化**します(日々変動・株式分割の反映のため)。個別分析は開くたびに最新化し、
>   **一覧(全銘柄)の株価も常駐の一括最新化サービス**が起動直後＋定期(既定6時間、Yahoo の一括quote APIを使用)で更新・保存します。
>   これにより、古い株価や分割前の価格が一覧に残りません(`appsettings` の `Fetch:PriceRefresh`/`PriceRefreshHours` で制御。手動更新は `POST /api/admin/price-refresh`)。
>   財務指標(PER/PBR/ROE 等)は会社予想ベースで変化が遅いためキャッシュし、「実データ更新」または週次巡回で更新します。
>   - **IR BANK**: 自己資本比率。
>   - **算出**: EPS(=株価/予想PER)・BPS・1株配当・配当性向・MIX係数。
>   - 営業利益率・増収率・利益成長率は、無料で正確に取得できる出典が無いため**未取得**のままにします(誤った値を出さない方針。取得できない値は「-」表示)。
> - **株主優待は自動取得の対象外**です(各サイトの優待情報は誤検出が避けられないため)。自動取得した銘柄は優待を「未取得」と表示し、
>   実データは **CSV取込** で反映します(優待列を含む CSV を取り込むと実データ優待として扱います)。
> - サーバ常駐の **自動取得バックグラウンドサービス**が robots.txt を順守し低頻度で **全銘柄**を巡回取得し、一覧も順次実データ化します。
>   オンデマンドと取得キャッシュ(`fetch_state.json`)を共有し二重取得を防ぎます。`appsettings` で有効/無効・間隔・対象範囲を制御できます。
> - データは **ローカル単独の WPF 版とは別に、サーバ側の `DataDir`**(既定 `DStockAnalysis.Web/data`)へ JSON 保存されます。
>   (注: WPF デスクトップ版は従来どおりローカルで擬似指標を生成します。サンプル値を使わないのは Web 版です。)
...
| キー | 既定 | 説明 |
|---|---|---|
| `Enabled` | `true` | バックグラウンド全銘柄巡回取得(財務指標)の有効/無効 |
| `OnDemand` | `true` | 個別分析で銘柄を開いた時に、その銘柄の実値をその場で取得する |
| `PriceRefresh` | `true` | 取得済み全銘柄の株価を定期一括最新化する(一覧の古い/分割前価格を防ぐ) |
